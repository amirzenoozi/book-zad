...
Note the flags are taken from the original `twitter/twemoji` repository, not the
`jdecked` fork — they differ at `1f1ee-1f1f7`, where the original is the Lion and
Sun flag used here for Persian (`lion-sun.svg`).
